...
namespace Ei.Persistence
{
    ...
    public abstract class InstitutionLoader : IDaoFactory
    {
        // abstract methods

        public abstract string SaveAccess(AccessConditionDao[] dao);

        public abstract T Deserialise<T>(string source);
        ...
        private static List<WorkflowConnection> ExtractWorkflowConnections(InstitutionDao dao, WorkflowDao workflow) {
            ...
            foreach (var connection in workflow.Connections.Where(w => !string.IsNullOrEmpty(w.ActionId))) {
                ...
                if (action == null) {
                    throw new Exception("Action does not exist: " + connection.ActionId);
                }
